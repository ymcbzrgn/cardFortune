...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        string finalCollectionName = Environment.GetEnvironmentVariable("MONGODB_FINALCOLLECTION_NAME") ?? string.Empty;

        var client = new MongoClient(connectionString);
        var database = client.GetDatabase(databaseName);
        var collection = database.GetCollection<BsonDocument>(collectionName);
        var finalCollection = database.GetCollection<BsonDocument>(finalCollectionName);

        var cardFortune = new CardFortune(collection,finalCollection);
